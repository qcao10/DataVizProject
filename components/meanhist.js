...
class MeanHist extends D3Component {
  ...
  initialize(node, props) {
   //this.color = "steelblue";
    ...
    var sd_mean = 1/Math.sqrt(1000);
    this.values = d3.range(1000).map(d3.randomNormal(0, sd_mean));
    ...
    var margin = {top: 20, right: 5, bottom: 30, left: 15},
  ...
  replot(mean,sd,samplesize){
    this.values = d3.range(samplesize).map(d3.randomNormal(mean, sd/Math.sqrt(samplesize)));
    //var this.values2 = d3.range(1000).map(d3.randomNormal(1, 1));
    //console.log(this.values)
    ...
    this.hist = this.svg.append("g")
        ...
        .attr("transform", "translate(0," + height + ")")
        .call(xAxis);
    // this.sampleMean = d3.mean(this.values).toFixed(4);
    // this.svg.append("text").attr("x", 400).attr("y", 60).text("Sample mean = " + this.sampleMean)
    // .style("font-size", "15px").attr("alignment-baseline","middle");
    }  
}

module.exports = MeanHist;
